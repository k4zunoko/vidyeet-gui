...
#### 0. Progressive Warmup パターン（初期フェーズ）

**問題:** 最初のチャンク完了まで（数秒～十数秒）プログレスバーが 0% のまま静止する → ユーザーは「動いているのか？」と不安になる

**Nielsen Norman Group の研究結果:**
- **即座のフィードバックが必須**: ユーザーアクション後、100ms 以内に何らかの反応が必要
- **進捗の速度変化**: ユーザーは敏感に察知し、満足度に影響
  - ❌ 最初速く→後遅い = 不満（期待を裏切る）
  - ✅ **最初遅く→後速く** = 満足（期待を超える）

**解決策: Progressive Warmup パターン**
1. **アップロード開始直後**: 0% で静止させず、すぐに進捗を開始
2. **最初の 2 秒間**: 時間駆動で 0% → 1.5% へ**ゆっくり**進む
3. **最初のチャンク完了後**: 実際の進捗（例: 16%）へジャンプし、以降は chunk ベースの補間へ移行

**メリット:**
- ✅ **即座のフィードバック**: アップロード開始直後から視覚的な変化
- ✅ **不確実性の軽減**: システムが動作中であることを保証
- ✅ **期待値管理**: 最初は遅いので、後で速くなっても期待を裏切らない
- ✅ **ほぼ正確**: 1.5% のみの "誇張" で誠実性を維持

**設定値:**
```typescript
WARMUP_TARGET_PERCENT = 0.015  // 1.5%（控えめな目標）
WARMUP_DURATION = 2000ms       // 2秒かけてゆっくり到達
```

**動作フロー:**
```
0s: Upload Start → Warmup タイマー開始
0-2s: 0% → 1.5% へゆっくり進む（easeOutCubic）
t秒: 1st Chunk Complete (例: 16%) → Warmup 停止、実際の進捗へ
以降: 既存の補間アルゴリズム（chunk 完了時間ベース）
```
...
- `src/composables/useProgressInterpolation.ts`: 補間ロジックのコンポーザブル
  - Progressive Warmup: `startUpload()`, `updateWarmup()`, `startWarmup()`, `stopWarmup()`
  - Chunk-based Interpolation: `updateTruth()`, `updateInterpolation()`
- `src/App.vue`: アップロード進捗コールバックで補間を適用

### 期待される UX 効果
1. **即座のフィードバック（Warmup）**: アップロード開始直後からプログレスバーが動く
2. **滑らかな進捗表示**: chunk ジャンプが緩和され、連続的に見える
3. **待ち時間の短縮感**: 動きがあることで認知リソースが分散
4. **期待値の管理**: 最初は遅く、後は速く → 期待を超えて満足度向上
5. **誤解の防止**: 推定であることを明示し、信頼性を維持
6. **境界での自然な停止**: ease-out により、停止が違和感なく見える
