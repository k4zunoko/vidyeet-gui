...
ライブラリ一覧の最上部に配置された専用アップロードボタン。

**デザイン原則:**
- Nielsen Norman Groupのアイコンユーザビリティ研究に基づき、常にテキストラベルを表示
- F字型スキャンパターンに最適化された配置（一覧トップ = 最初に目に入る）
- 動画カードと視覚的に明確に区別（点線ボーダー、異なる背景色）

**表示仕様:**
- 位置: 動画グリッドの最上部（スクロール対象内）
- アスペクト比: 16:9（動画カードと統一してグリッドの一貫性を保つ）
- デザイン:
  - 2px点線ボーダー（`var(--color-border)`）
  - 背景: `var(--color-surface)`
  - 中央に大きな+アイコン（40px）とテキスト「動画をアップロード」
- ホバー状態:
  - ボーダー色: プライマリカラー（`var(--color-primary)`）
  - 浮き上がり効果: `translateY(-2px)` + 影
  - 背景: `var(--color-surface-hover)`
- トランジション: 0.2秒 cubic-bezier（Doherty Threshold準拠）

**インタラクション:**
- クリック: ファイル選択ダイアログを開く（単一または複数選択可能）
- キーボード: Enter/Spaceキーで実行可能（`.prevent`修飾子で2重実行防止）
- アクセシビリティ:
  - `role="button"`
  - `aria-label="動画をアップロード"`
  - `:focus-visible` でアウトライン表示

**UXチェック（Upload Button）**
- **Nielsen Norman Group準拠**: アイコンには必ずテキストラベルを付ける
- **F字型スキャン**: トップ配置で最初に目に入る（高視認性）
- **視覚的階層**: 点線ボーダーで通常の動画カードと明確に区別
- **Fitts' Law**: 十分なクリック領域（16:9比率全体）
- **認知負荷**: シンプルなアイコン+テキストで意図が即座に理解可能
- **決断疲れ**: 常に同じ位置に表示されることで操作が習慣化
...
- タイトルバーの更新ボタンで一覧を再読み込み
- タイトルバーの設定ボタンで設定モーダルを開く
...
設定モーダルは、ユーザーアカウント管理、アップデートの手動操作、アプリ情報を表示するダイアログです。

### 設計方針（モーダル）

Nielsen Norman Groupの人間工学的知見に基づき、**モーダルダイアログ**として実装します。

**モーダルを採用する理由:**
- シンプルで少数の設定項目に適している（NN/g）
- 破壊的操作（ログアウト）に集中させるため、他の操作をブロックするのが適切
- 短時間で完了する操作（数秒以内）

### 表示仕様

- **位置**: 画面中央
- **z-index**: 200（アップロードダイアログより高く、トーストより低い）
- **背景**: 半透明のオーバーレイ（80%黒）でモーダル性を強調
- **アニメーション**: フェードイン（0.2秒）
- **サイズ**: 最大幅480px、コンテンツに応じた高さ

### 開始トリガー

- タイトルバーの「設定」ボタン（歯車アイコン）をクリック

### 表示内容

**セクション1: アカウント**
- セクションタイトル: 「アカウント」（小文字、ミュート色）
- ログアウトボタン:
  - テキスト: 「ログアウト」
  - スタイル: 赤色（警告色）、最小高44px（Fitts' Law準拠）
  - 視覚的階層: 最も目立つ要素（破壊的操作のため）
...
- 状態はカード内に集約して表示
- ステータス:
  - 未確認 / 確認中 / 更新あり / 最新 / ダウンロード中 / 準備完了 / エラー
- 主要アクション:
  - 更新を確認（`update:check`）
  - 更新あり → ダウンロード（`update:download`）
  - 準備完了 → インストールして再起動（`update:install`）
- 進捗表示:
  - ダウンロード中はパーセンテージと転送量を表示
  - 進捗バーは percent-done indicator を使用
- エラー:
  - 画面内で原因と次の行動（再試行）を提示
  - 開発ビルドでは「パッケージ版のみ対応」の文言を表示

**セクション3: アプリケーション情報**
- セクションタイトル: 「アプリケーション」（小文字、ミュート色）
- バージョン: package.jsonから自動取得
- CLIパス: `bin/vidyeet-cli.exe`

### 閉じ方（3通り）

1. **×ボタン**: モーダル右上
2. **Escキー**: キーボードショートカット
3. **オーバーレイクリック**: 背景の半透明部分をクリック

### UXチェック（Settings Modal）

- **Nielsen Norman Group準拠**: シンプルな設定にはモーダルが適切
- **認知負荷**: セクションごとにグループ化（アカウント、アップデート、アプリ情報）
- **視覚的階層**: セクションタイトル（小、ミュート）→ アイテム → ボタン（大、カラフル）
- **エスケープルート**: 3通りの閉じ方で柔軟性を確保（NN/g）
- **Fitts' Law**: ログアウトボタンは最小高44px
- **ドハティの閾値**: 0.2秒のアニメーション
- **フレーミング効果**: ログアウトは赤色で警告性を強調
- **システム状態の可視化**: 更新ステータスを明示し、次の行動を迷わせない

### 操作機能

- **ログアウト**: クリックで認証情報をクリアし、ログイン画面に戻る
- **更新確認**: 手動で更新を確認し、必要に応じてダウンロード・再起動する
- **情報表示のみ**: バージョンとCLIパスは読み取り専用
...
- ライブラリ一覧上部のアップロードボタン（常時表示）をクリック
