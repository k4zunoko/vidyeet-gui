...
> **✅ 移行完了済み (2025年)**  
> CLIのJSON出力形式は修正され、GUI側のパース処理も更新されました。  
> このドキュメントは記録として保持されています。

## 概要

CLIの`upload --progress`コマンドにおいて、進捗情報のJSON出力形式に不要なネストが存在するバグが発見され、修正されました。このドキュメントでは、バグ修正に伴うGUI側の変更箇所と修正方針をまとめます。
